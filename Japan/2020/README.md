...
2. ゲーム以外のアプリである。  
ゲームカテゴリに公開されているアプリは全て対象外となる。  
ただしゲームカテゴリ以外で公開されており、知育やクイズなど、遊びが主題ではないものは調査対象に含む
3. 最新のiOSのiPhoneにインストールが可能である
4. 無料アプリである
5. アプリが公開されている

アプリの一覧は[CatchApp](https://catchapp.net)より、「新着アプリから探す」の「総合」で「リリース日」順にし、「無料のみ」「日本語」を選択したものを使用している。  
...
## フレームワークの判別
### パスによる判別
以下のフレームワークは、解凍したバックアップのパスにおいて、それぞれ以下の正規表現に合致する場合、そのフレームワークを用いて開発をしていると判断した。
...
その為、WebView主体アプリに関しては正しい統計の実現は不可能である。あくまでも目安として判断していただきたい。
...
その為アプリを特定できる様な情報は載せておらず、リバースエンジニアリングによって得られた情報は統計の作成用途でのみ用いている。
